...
func main() {
	...
	flag.StringVar(&kubectlExectuable, "kubectl", "/kubectl", "The path to kubectl executable.")
	opts := zap.Options{
		Development: true,
	}
	...
	mgr, err := ctrl.NewManager(ctrl.GetConfigOrDie(), ctrl.Options{
		...
		LeaderElectionID:       "df820e3e.pixiv.net",
	...
	kubectlRunner := kubectl.NewCommand(kubectlExectuable)
	patchRunner := kustomize.NewPatchRunner(kubectlRunner)

	if err = (&controller.PodProfileReconciler{
		Client: mgr.GetClient(),
		Scheme: mgr.GetScheme(),
	}).SetupWithManager(mgr); err != nil {
		setupLog.Error(err, "unable to create controller", "controller", "PodProfile")
		os.Exit(1)
	}
