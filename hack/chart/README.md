...
You can generate the Helm chart for the CRD by running the following command: `./make.sh ${CHART_VERSION} ${IMAGE_NAME} ${IMAGE_TAG}`.
...
Running the script as `./make.sh ${CHART_VERSION} ${IMAGE_NAME} ${IMAGE_TAG} ${DESTINATION}` will generate the file `${DESTINATION}/k8s-job-wrapper-${CHART_VERSION}.tgz`.
